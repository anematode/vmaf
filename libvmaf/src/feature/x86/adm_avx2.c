...
static inline __m256i double_to_64(__m256d v) {
    __m256i v_lo = _mm256_blend_epi32(_mm256_set1_epi64x(0x4330000000000000 /* 2**52 */), (__m256i)v, 0x55);
    __m256i v_hi = _mm256_srli_epi64((__m256i)v, 32);
    v_hi = _mm256_xor_si256(v_hi, _mm256_set1_epi64x(0x4530000080000000 /* 2**84 + 2**63 */));

    __m256d v_hi_dbl = _mm256_sub_pd(
        _mm256_castsi256_pd(v_hi),
        _mm256_set1_pd(1.9342822341709703e+25 /*2**84 + 2**63 + 2**52*/));
    return (__m256i)_mm256_add_pd(v_hi_dbl, _mm256_castsi256_pd(v_lo));
}
